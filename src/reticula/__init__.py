...
from ._reticula_ext import int64, double, string
...
pair = _generic_attribute(
    ...
    options={(i, j) for i in _simple_vertex_types for j in _simple_vertex_types},
...
for _dist in ["geometric_distribution", "uniform_int_distribution"]:
    setattr(
        _sys.modules[__name__],
        _dist,
        _generic_attribute(
...
for _e in _temporal_edge_prefixes:
    setattr(
        ...
        _generic_attribute(
            ...
            options={(v, t) for v in _all_vertex_types for t in _time_types},
            function_module=_reticula_ext,
            api_module_name=__name__,
        ),
    )
...
_vertex_generic_attrs = ["component", "component_size", "component_size_estimate"]
...
from ._reticula_ext import is_graphic, is_digraphic
...
def to_networkx(network, create_using=None):
    ...
    if not is_dyadic[edge_type]():
        raise ValueError(
            "only dyadic networks can be translated into " "NetworkX graphs"
        )
    if is_temporal_edge[edge_type]():
        raise ValueError(
            "only static networks can be translated into " "NetworkX graphs"
        )
...
for _vert_t in _all_vertex_types:
    setattr(
        _sys.modules[__name__],
        f"_from_networkx_{_vert_t.__name__}",
